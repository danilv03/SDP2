 ...
 *
 * L1 D-cache configuration: LRFU eviction, 4-way associativity, 64B block
 * size, 64 sets → 16 KB total capacity.
 *
 * L2 D-cache configuration: LRFU eviction, 8-way associativity, 64B block
 * size, 512 sets → 256 KB total capacity. Private per core, inclusive of L1.
 *
 * Two-level access logic (vcpu_mem):
 *   L1 hit                → silent return; L2 LRFU state updated.
...
typedef struct {
    CacheBlock *blocks;
    double   *crf;              /* LRFU Combined Recency and Frequency value  */
    uint64_t *last_access_time; /* per-block timestamp of last access         */
    uint64_t  time_counter;     /* per-set monotonic logical clock            */
} CacheSet;
 ...
 * cpus[cpu_index].arith_count inside shared memory (ldst-trace-final.c). */

static Cache **l1_dcaches;
static Cache **l2_dcaches;

/* Session tracking: per-CPU flag that records whether logging was active on
 * the previous vcpu_mem call. Used to detect the log-on transition so
 * arith_count and insn_count in shared memory are reset at session start.
...
/* ---------------------------------------------------------------------------
 * LRFU (Least Recently/Frequently Used) replacement policy.
 *
 ...
 *
 * Each block b carries a Combined Recency and Frequency (CRF) value.
 * The weighting function is F(x) = (1/p)^(λx) with p = 2:
 *
 *   On hit  :  CRF(b) = F(0) + F(Δ) · CRF_prev(b)   where Δ = t_c − t_last
 *              = 1 + 2^(−λΔ) · CRF_prev(b)
 *
 *   On install (miss): CRF(b) = 1.0   (first reference)
 *
 *   Eviction: select the block with the smallest *effective* CRF, computed as
 *             effective_CRF(b, t_c) = F(t_c − t_last(b)) · CRF(b)
 *             = 2^(−λ(t_c − t_last(b))) · CRF(b)
 *
 * Spectrum property:
 *   λ → 0 : F(x) → 1 for all x, CRF ≈ access count → pure LFU.
 *   λ → ∞ : F(x) → 0 for x > 0, only the most recent access matters → LRU.
 *   0 < λ < 1 : hybrid that weights both recency and frequency.
 *
 * The default LRFU_LAMBDA = 0.5 gives a balanced midpoint.
 * --------------------------------------------------------------------------- */
#define LRFU_LAMBDA       0.5   /* tunable: 0 → LFU, large → LRU            */
#define LRFU_BASE         2.0   /* p in the weighting function F(x) = p^(-λx)*/

static void lrfu_priorities_init(Cache *cache)
{
    int i;

    for (i = 0; i < cache->num_sets; i++) {
        cache->sets[i].crf              = g_new0(double,   cache->assoc);
        cache->sets[i].last_access_time = g_new0(uint64_t, cache->assoc);
        cache->sets[i].time_counter     = 0;
    }
}

/**
 * lrfu_hit_blk(): Update LRFU metadata on a cache hit.
 *
 * Accumulates the CRF using the recursive LRFU formula:
 *   CRF(b) = 1 + 2^(-λΔ) · CRF_prev(b)
 * which captures both recency (via the exponential decay) and frequency
 * (via the additive +1 for each access).
 */
static void lrfu_hit_blk(Cache *cache, int set_idx, int blk_idx)
{
    CacheSet *set = &cache->sets[set_idx];
    uint64_t tc    = set->time_counter;
    uint64_t delta = tc - set->last_access_time[blk_idx];

    double decay = pow(LRFU_BASE, -LRFU_LAMBDA * (double)delta);
    set->crf[blk_idx] = 1.0 + decay * set->crf[blk_idx];
    set->last_access_time[blk_idx] = tc;
    set->time_counter++;
}

/**
 * lrfu_install_blk(): Initialise LRFU metadata for a newly inserted block.
 *
 * A cold block enters with CRF = 1.0 (a single reference). This prevents
 * newly fetched blocks from immediately becoming eviction victims while
 * still giving long-resident frequently-accessed blocks higher CRF.
 */
static void lrfu_install_blk(Cache *cache, int set_idx, int blk_idx)
{
    CacheSet *set = &cache->sets[set_idx];
    set->crf[blk_idx]              = 1.0;
    set->last_access_time[blk_idx] = set->time_counter;
    set->time_counter++;
}

/**
 * lrfu_get_victim_block(): Select the eviction victim via minimum effective CRF.
 *
 * The effective CRF ages each block's stored CRF to the current time:
 *   effective_CRF(b) = 2^(-λ(t_c - t_last(b))) · CRF(b)
 * The block with the smallest value is the one whose combination of
 * recency and frequency is weakest — the LRFU victim.
 */
static int lrfu_get_victim_block(Cache *cache, int set_idx)
{
    CacheSet *set = &cache->sets[set_idx];
    uint64_t tc = set->time_counter;

    double min_effective_crf = DBL_MAX;
    int    min_idx = 0;
    int    i;

    for (i = 0; i < cache->assoc; i++) {
        uint64_t delta = tc - set->last_access_time[i];
        double decay = pow(LRFU_BASE, -LRFU_LAMBDA * (double)delta);
        double effective_crf = decay * set->crf[i];
        if (effective_crf < min_effective_crf) {
            min_effective_crf = effective_crf;
            min_idx = i;
        }
    }
...
static void lrfu_priorities_destroy(Cache *cache)
{
    int i;

    for (i = 0; i < cache->num_sets; i++) {
        g_free(cache->sets[i].crf);
        g_free(cache->sets[i].last_access_time);
    }
}
...
/* cache_init: LRFU eviction — initialise per-set CRF and timestamp arrays. */
...
/**
 * access_cache(): Simulate a cache access with LRFU eviction.
 * @cache: The cache under simulation
 * @addr: The address of the requested memory location
 *
 * Returns true if hit, false if miss. On a hit the accessed block's CRF
 * is updated via the LRFU recurrence. On a miss the LRFU victim (block
 * with minimum effective CRF) is evicted and the new block installed
 * with CRF = 1.0.
 */
static bool access_cache(Cache *cache, uint64_t addr)
{
    ...
    if (hit_blk != -1) {
        lrfu_hit_blk(cache, set, hit_blk);
        return true;
    }
    ...
    if (replaced_blk == -1) {
        replaced_blk = lrfu_get_victim_block(cache, set);
    }

    lrfu_install_blk(cache, set, replaced_blk);
...
/* ---------------------------------------------------------------------------
 * l2_install_block(): Install addr into the L2 cache (LRFU eviction).
 *
...
static uint64_t l2_install_block(Cache *l2, uint64_t addr)
{
    ...
    /* L2 hit: update CRF via LRFU recurrence, no eviction. */
    int hit_blk = in_cache(l2, addr);
    if (hit_blk != -1) {
        lrfu_hit_blk(l2, set, hit_blk);
        return 0;
    }
    ...
    if (victim == -1) {
        /* All ways valid — evict the LRFU victim (min effective CRF).
        ...
        victim = lrfu_get_victim_block(l2, set);
        evicted_addr = l2->sets[set].blocks[victim].tag
                       | (set << l2->blksize_shift);
    }

    lrfu_install_blk(l2, set, victim);
...
static void l1_back_invalidate(Cache *l1, uint64_t evicted_addr)
{
    ...
    if (way != -1) {
        ...
        /* Do NOT update CRF/timestamp: the invalid way will be found by
         * get_invalid_block() on the next L1 miss to this set, and
         * lrfu_install_blk() will overwrite its metadata then. */
    }
...
static void vcpu_mem(unsigned int cpu_index, qemu_plugin_meminfo_t info,
   ...
   * Reset both arith_count and insn_count in shared memory to 0 so this
   * session starts from a clean slate. logger.c reads both at shutdown. */
   ...
   *
   * L1 hit:
   *   Update L2 LRFU metadata for this block so L2 replacement state stays
   *   consistent with L1 (required for correct inclusive eviction ordering).
   ...
   *   block is now only in L2, which is fine.
   *   Return — not an L2 miss, nothing to record.
   *
   * L1 miss, L2 miss:
   *   Install block in L2 first. If L2 evicts a valid block, back-invalidate
   *   that block from L1 to maintain the inclusive invariant.
   *   Then install block in L1 (L1 eviction is safe — block is already in L2).
  ...
  if (hit_in_l1) {
    /* L1 hit: keep L2 LRFU state consistent — update CRF without recording miss. */
  ...
  if (hit_in_l2) {
    /* L2 hit: update LRFU state, then refill L1. No LogRecord needed. */
    l2_install_block(l2_dcaches[cache_idx], effective_addr);
 ...
 *
 * Kept from ldst-trace-final.c: atomically increments arith_count in shared
 * memory instead of using a private scoreboard, so logger.c can read it live.
...
/* Kept from ldst-trace-final.c: no arith_count_score to free since the
 * scoreboard was replaced by the shared memory field. */
...
QEMU_PLUGIN_EXPORT int qemu_plugin_install(qemu_plugin_id_t id,
   ...
   *   Sets         : 256
   *   Total size   : 64 * 4 * 256 = 64 KB
   *   Replacement  : LRFU (λ = 0.5)
   */
  l1_dassoc     = 4;
  l1_dblksize   = 64;
  l1_dcachesize = l1_dblksize * l1_dassoc * 256;  /* 65 536 B = 64 KB */
   ...
   *   Sets         : 4096
   *   Total size   : 64 * 8 * 4096 = 2 MB
   *   Replacement  : LRFU (λ = 0.5)
   *   Inclusion    : inclusive of L1 (back-invalidation on L2 eviction)
   */
  l2_dassoc     = 8;
  l2_dblksize   = 64;
  l2_dcachesize = l2_dblksize * l2_dassoc * 4096; /* 2 097 152 B = 2 MB */
  ...
  /* arith_count_score allocation intentionally absent (ldst-trace-final.c):
   * the count lives in cpus[i].arith_count, zeroed by memset above. */
