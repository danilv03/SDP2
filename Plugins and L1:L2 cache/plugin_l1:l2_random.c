 ...
 *
 * L1 D-cache configuration: Random eviction, 4-way associativity,
 * 64B block size, 64 sets → 16 KB total capacity.
 *
 * L2 D-cache configuration: Random eviction, 8-way associativity,
 * 64B block size, 512 sets → 256 KB total capacity. Private per core,
 * inclusive of L1.
 *
 * Replacement policy: Random (both levels).
 *
 * Design rationale:
 *   Random eviction picks a uniformly random way using GLib's PRNG. No
 *   per-hit or per-miss metadata is maintained per set — eviction is fully
 *   stateless. Random is the standard unbiased baseline: it avoids the
 *   pathological worst-case behaviour of LRU under cyclic access patterns
 *   while requiring zero per-set bookkeeping overhead.
 *
 * Two-level access logic (vcpu_mem):
 *   L1 hit                → no metadata to update (Random is stateless);
 *                           assert L2 inclusion via l2_promote(); return.
 *   L1 miss, L2 hit       → l2_install_block() hit path is a no-op (Random
 *                           hits touch no metadata); refill L1; return.
 *   L1 miss, L2 miss      → l2_install_block() evicts a random L2 block if
 *                           all ways are occupied, back-invalidates L1,
 *                           refills L1, emits LogRecord into shared memory.
 *
 ...
 * and (b) back-invalidating L1 whenever L2 evicts a valid block.
 */
...
/* CHANGED (from ldst-trace-random.c): lru_priorities and lru_gen_counter
 * fields removed from CacheSet — random eviction is stateless and needs
 * no per-set metadata. */
typedef struct {
    CacheBlock *blocks;
    /* Random eviction is stateless: no priority or timestamp arrays needed. */
} CacheSet;
 ...
 * the previous vcpu_mem call. Used to detect the log-on and log-off
 * transitions so arith_count can be reset at the start of each new session.
...
/* ---------------------------------------------------------------------------
 * CHANGED (from ldst-trace-random.c): Random replacement policy helper.
 *
 * Picks a uniformly random way index within [0, assoc) using GLib's PRNG.
 * No per-set state is needed — random eviction is entirely stateless.
 * GLib auto-seeds its global PRNG, so no explicit seeding is required.
 * Replaces lru_priorities_init, lru_update_blk, lru_get_lru_block, and
 * lru_priorities_destroy which are all removed.
 * --------------------------------------------------------------------------- */
static int get_random_block(Cache *cache, uint64_t set_idx)
{
    return g_random_int_range(0, cache->assoc);
}
...
/* CHANGED (from ldst-trace-random.c): lru_priorities_init() call removed —
 * random eviction carries no per-set state to initialise. */
static Cache *cache_init(int blksize, int assoc, int cachesize)
{
    ...
    /* No init call needed: random eviction carries no per-set state. */
...
/**
 * access_cache(): Simulate a cache access with random eviction.
 * @cache: The cache under simulation
 * @addr: The address of the requested memory location
 *
 * Returns true on a hit, false on a miss. On a miss the incoming block is
 * installed, evicting a randomly chosen block when all ways are valid.
 *
 * CHANGED (from ldst-trace-random.c): removed lru_update_blk() call on hit,
 * removed lru_get_lru_block() and second lru_update_blk() call on miss.
 * Replaced with get_random_block(). Hit path now returns immediately with
 * no state update since random eviction is fully stateless.
 */
static bool access_cache(Cache *cache, uint64_t addr)
{
    ...
    if (hit_blk != -1) {
        /* Random eviction is stateless; no updates needed on a hit. */
        return true;
    }

    /* Miss: prefer an invalid (empty) way; fall back to random eviction. */
    replaced_blk = get_invalid_block(cache, set);

    if (replaced_blk == -1) {
        replaced_blk = get_random_block(cache, set);
    }
 ...
 *
 * Called on the L1-hit path. Under Random eviction, hits update no metadata
 * — eviction is fully stateless — so there is nothing to do beyond verifying
 * that the block is present in L2 (inclusion invariant check).
 *
 * The g_assert fires if the block is in L1 but absent from L2, indicating an
 * inclusion invariant violation.
 * --------------------------------------------------------------------------- */
static void l2_promote(Cache *l2, uint64_t addr)
{
    ...
    /* Random: no metadata update on a hit. */
}

/* ---------------------------------------------------------------------------
 * l2_install_block(): Install addr into the L2 cache (Random eviction).
 *
 ...
 * present). The caller passes this address to l1_back_invalidate().
 *
 ...
 *
 * On an L2 hit, 0 is returned immediately — Random hits are no-ops.
 * On an L2 miss with all ways valid, a random victim is chosen.
 *
...
static uint64_t l2_install_block(Cache *l2, uint64_t addr)
{
    ...
    /* L2 hit: Random eviction does not update any metadata on a hit. */
    int hit_blk = in_cache(l2, addr);
    if (hit_blk != -1) {
        return 0;
    }
    ...
    if (victim == -1) {
        /* All ways valid — evict a random block.
        ...
        victim = get_random_block(l2, set);
 ...
 *
 * evicted_addr == 0 means no valid block was displaced from L2, nothing to do.
 *
 ...
 *
 * Random eviction carries no per-way metadata so there is nothing to reset
 * on invalidation beyond clearing the valid flag.
 * --------------------------------------------------------------------------- */
...
static void vcpu_mem(unsigned int cpu_index, qemu_plugin_meminfo_t info,
   ...
   *
   * L1 hit:
   *   Random eviction is stateless — no metadata to update on a hit.
   *   Call l2_promote() to assert the inclusion invariant. Return.
   *
   * L1 miss, L2 hit:
   *   l2_install_block() hits the L2 hit path — returns 0, no eviction, no
   *   metadata update. Refill L1 via access_cache(). Return.
   *
   * L1 miss, L2 miss:
   *   l2_install_block() runs the miss path — evicts a random L2 block if
   *   all ways are occupied, reconstructs evicted_addr, installs new block.
   *   Back-invalidate L1 if evicted_addr != 0. Refill L1. Emit LogRecord.
   * ---------------------------------------------------------------------- */
  bool hit_in_l1 = access_cache(l1_dcaches[cache_idx], effective_addr);

  if (hit_in_l1) {
    /* L1 hit: assert inclusion invariant. No metadata update (Random). */
...
/* CHANGED (from ldst-trace-random.c): lru_priorities_destroy() call removed —
 * no per-set state was allocated so none needs freeing. */
static void cache_free(Cache *cache)
{
    ...
    /* No destroy call needed: random eviction carries no per-set state. */
