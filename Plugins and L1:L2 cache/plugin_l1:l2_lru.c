 ...
 *
 * L1 D-cache configuration: LRU eviction, 4-way associativity, 64B block
 * size, 64 sets → 16 KB total capacity.
 *
 * L2 D-cache configuration: LRU eviction, 8-way associativity, 64B block
 * size, 512 sets → 256 KB total capacity. Private per core, inclusive of L1.
 *
 * Two-level access logic (vcpu_mem):
 *   L1 hit                → silent return; L2 LRU updated for recency.
...
/* cache_init: LRU eviction — initialise per-set priority arrays. */
...
/**
 * access_cache(): Simulate a cache access with LRU eviction.
 * @cache: The cache under simulation
 * @addr: The address of the requested memory location
 *
 * Returns true if hit, false if miss. On a hit the accessed block is
 * promoted to MRU position. On a miss the LRU block is evicted.
 */
...
/* ---------------------------------------------------------------------------
 * l2_install_block(): Install addr into the L2 cache (LRU eviction).
 *
...
static uint64_t l2_install_block(Cache *l2, uint64_t addr)
{
    ...
    /* L2 hit: promote to MRU, no eviction. */
    int hit_blk = in_cache(l2, addr);
    if (hit_blk != -1) {
        lru_update_blk(l2, set, hit_blk);
        return 0;
    }
    ...
    if (victim == -1) {
        /* All ways valid — evict the LRU block.
...
static void l1_back_invalidate(Cache *l1, uint64_t evicted_addr)
{
    ...
    if (way != -1) {
        ...
        /* Do NOT update lru_priorities: the invalid way will be found by
         * get_invalid_block() on the next L1 miss to this set, and
         * lru_update_blk() will overwrite its priority then. */
    }
...
static void vcpu_mem(unsigned int cpu_index, qemu_plugin_meminfo_t info,
   ...
   *
   * L1 hit:
   *   Promote L2 LRU entry for this block so L2 recency stays consistent
   *   with L1 (required for correct inclusive LRU eviction ordering).
  ...
  if (hit_in_l1) {
    /* L1 hit: keep L2 LRU order consistent — promote without recording miss. */
  ...
  if (hit_in_l2) {
    /* L2 hit: promote L2 LRU, then refill L1. No LogRecord needed. */
    l2_install_block(l2_dcaches[cache_idx], effective_addr);
